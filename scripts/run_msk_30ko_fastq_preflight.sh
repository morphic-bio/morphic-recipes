#!/usr/bin/env bash
# MSK 30KO FASTQ preflight bundle.
#
# This is intentionally read-only with respect to FASTQs. It checks that each
# manifest row lands in the expected 10x whitelist family/namespace before any
# STAR production run is launched.
...
MANIFEST="${MANIFEST:-${REPO_ROOT}/docs/MSK_30KO_FASTQ_MANIFEST.tsv}"
OUTDIR="${OUTDIR:-${REPO_ROOT}/plans/artifacts/msk_30ko_fastq_preflight_$(date +%Y%m%d_%H%M%S)}"
...
echo
echo "MSK FASTQ preflight outputs:"
